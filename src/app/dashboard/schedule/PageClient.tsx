...
export default function SchedulerApp() {
  ...
  // Locked clients stay in their position; only unlocked ones are reordered.
  const optimizeRoute = () => {
    ...
    // If fewer than 2 unlocked clients, nothing to optimise
      ...
      (result, status) => {
        if (status === google.maps.DirectionsStatus.OK && result) {
          ...
          if (optimizedOrder) {
            ...
            // Merge locked clients back into their original positions
            const finalOrder: Client[] = [];
            let unlockedIdx = 0;
            for (let i = 0; i < activeTeam.clients.length; i++) {
              if (lockedPositions[i]) {
                finalOrder.push(lockedPositions[i]);
              } else {
                finalOrder.push(reorderedUnlocked[unlockedIdx++]);
              }
            }
